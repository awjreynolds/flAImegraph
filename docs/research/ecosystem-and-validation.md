...
## Harness candidates and an additional protocol boundary

Follow-up checked 2026-09-06. Pi and Codex are proposed evaluation targets, not selected integrations. The user's term “MyPi” remains ambiguous: Oh My Pi and other similarly named projects exist. Do not select a repository silently.

Pi documents extension lifecycle events and final assistant-message usage. Its RPC documentation distinguishes streaming cumulative updates from the authoritative final message. An existing third-party `pi-otlp` extension exports metrics with Pi-specific names, including token and cost counters. This is useful reuse evidence, but neither its documentation nor the OTLP transport establishes complete per-call trace ancestry or GenAI semantic compatibility. [Pi extensions](https://pi.dev/docs/latest/extensions), [Pi RPC](https://pi.dev/docs/latest/rpc), [pi-otlp](https://github.com/mikeyobrien/pi-otlp).

Codex documents opt-in OTel export, including conversation metadata, API attempts, and token counts on completed SSE responses. This review has not enabled it or verified complete capture of this desktop conversation, subagents, and every transport path. [Official Codex configuration](https://learn.chatgpt.com/docs/config-file/config-advanced).

Oh My Pi documents its own extension system and a local usage-statistics package. It would be an additional implementation compatibility check if confirmed as the intended target; similarities with Pi do not prove identical usage semantics. [Oh My Pi](https://github.com/can1357/oh-my-pi).

**Agent Client Protocol (ACP) belongs in the standards assessment.** Its current v1 schema includes `UsageUpdate` for context-window occupancy and optional cumulative session cost. Its completed session-usage RFD explicitly separates that scope from an end-turn-token-usage RFD that remains Draft. Context occupancy is therefore not a substitute for accumulated input/output consumption. An open issue also reports divergent turn/session interpretations in existing implementations; that report is evidence to investigate, not a universal claim about ACP implementations. [ACP schema](https://agentclientprotocol.com/protocol/v1/schema#usageupdate), [completed session usage RFD](https://agentclientprotocol.com/rfds/session-usage), [draft end-turn usage RFD](https://agentclientprotocol.com/rfds/end-turn-token-usage), [usage semantics issue](https://github.com/agentclientprotocol/agent-client-protocol/issues/1860).

ACP addresses the client-to-agent boundary, OTLP telemetry export, MCP tool/context access, and FOCUS financial records. Their applicability must be assessed separately. A candidate pair of Pi and Codex offers different harness implementations for comparison, while an ACP adapter might offer a shared capture surface where it exposes sufficient information. Neither choice has yet been validated against real records.
