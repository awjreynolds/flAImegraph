...
Regenerate with `node --import tsx tools/dogfood-usage-v04.ts` after installing dependencies. To regenerate only the native derived profiles from the existing browser capture, use `node --import tsx tools/dogfood-usage-v04.ts --native-only`. The generators use public metadata-only artifacts and deterministic illustrative inputs; they do not contact a provider.
...
- `*.folded`, `*.pprof`, `*.svg`: standard single-meter profiles. Native selects input tokens grouped by task and operation; files selects operation count using real ancestry.
- `usage-task-example.json`: **synthetic declared** three-task token breakdown generated by `node --import tsx tools/dogfood-task-usage.ts`. It contains explicit task IDs, cached-input subsets and two unavailable reasoning-token measurements; no provider call or live token usage is asserted.
