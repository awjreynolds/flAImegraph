...
export const EFFICIENCY_SCHEMA_VERSION = "0.4.0" as const;
...
export interface EfficiencyFinding {
  ...
  work_item_ids?: string[];
  observation_ids?: string[];
  measurement?: ExactRatio & { unit: string };
  evidence?: EfficiencyEvidence;
  source_refs?: UsageSourceRef[];
  limitations?: string[];
}
...
export interface BenchmarkComparison {
  ...
  sample_sizes: { baseline: string; candidate: string };
  accepted_counts: { baseline: string; candidate: string };
  acceptance_rate_delta: string | null;
  resource_deltas: BenchmarkResourceDelta[];
  latency_ns: { baseline: ExactRatio | null; candidate: ExactRatio | null; delta: string | null };
  limitations: string[];
  source_refs: UsageSourceRef[];
}
