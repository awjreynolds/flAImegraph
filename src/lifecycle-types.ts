...
export const LIFECYCLE_SCHEMA_VERSION = "0.5.0" as const;
...
export type LifecycleData =
  | { kind: "epoch" }
  | ({ kind: "start" } & LifecycleActionInput)
  | { kind: "pause"; action_id: string; reason: InterruptionReason }
  | { kind: "resume"; action_id: string }
  | { kind: "heartbeat"; action_id: string }
  | { kind: "end"; action_id: string; status: Exclude<UsageStatus, "running">; reason?: InterruptionReason }
  | { kind: "measurement"; action_id: string; meter: UsageMeter; measurement: Omit<UsageMeasurement, "source_refs"> };
...
export interface LifecycleAction {
  ...
  gaps: { from: string; to: string; elapsed_ns: string }[];
