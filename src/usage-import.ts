...
function legacy(input: unknown, options: LegacyEvidenceImportOptions = {}): UsageBundle {
  ...
  for (const row of legacyRows) {
    ...
    observations.push(baseObservation({ id, sourceId: rowSourceId, recordRef: ref, subject: firstString(row.operation) ?? (row.kind === "tool" ? "tool" : row.kind === "activity" ? "activity" : "model.response"), accounting: accountingScope(row), operationId: firstString(row.operation), parentId: parentNative === null ? null : observationId({ dataset_id: datasetId }, "legacy", parentNative), agentId: firstString(row.agent_id) ?? options.agent_id, sessionId: firstString(row.session_id) ?? options.session_id, taskId: firstString(row.task_id) ?? options.task_id, workItemId: workItemId(options, row), status: rawStatus(row), event: row.timestamp, ended: row.end_time, collected: options.collected_at, measurements, dimensions }));
  }
