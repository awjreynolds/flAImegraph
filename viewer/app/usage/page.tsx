...
import { createUsageReport, validateUsageBundle, validateUsageReport, importUsage, fromLegacyEvidence, analyzeEfficiency, decimalCompare, createUsageProfile } from '@/lib/usage.js';
import type { UsageBundle, UsageGrouping, UsageObservation, UsageDimensionFact, EfficiencyOptions } from '@/lib/usage.js';

const names: Record<string, string> = { work_item: 'Work identifier', input_tokens: 'Input tokens', output_tokens: 'Output tokens', cache_read_input_tokens: 'Cached input', cache_write_input_tokens: 'Cache writes', reasoning_output_tokens: 'Reasoning output', operation_count: 'Operations', read_bytes: 'Bytes read', returned_bytes: 'Bytes returned', elapsed_ns: 'Elapsed intervals', duration_ns: 'Compute duration', tool_calls: 'Tool calls' };
...
const examples = { native: { path: '/usage-native.json', label: 'Recorded development session' }, files: { path: '/usage-files.json', label: '5,000-file operation capture' } };
...
function loadedBundle(value: unknown): UsageBundle {
  if (value && typeof value === 'object' && 'bundle' in value) return validateUsageReport(value).bundle;
  if (value && typeof value === 'object' && 'evidence' in value) return validateUsageBundle(fromLegacyEvidence(value.evidence));
  if (value && typeof value === 'object' && 'schema_version' in value && value.schema_version === '0.1.0') return validateUsageBundle(fromLegacyEvidence(value));
  return validateUsageBundle(value);
}
...
export default function UsageExplorer() {
  ...
  function adopt(next: UsageBundle, name: string) {
    const first = next.meters.find(meter => meter.id === 'input_tokens') ?? next.meters.find(meter => meter.id === 'operation_count') ?? next.meters[0];
    setBundle(next); setGroup(next.observations.some(row => row.work_item_id !== null) ? 'work_item' : 'model'); setFilename(name); setMetric(first?.id ?? ''); setSelected(null); setSelectedGroup(null); setQuery(''); setPage(0); setAnalysisOptions({}); setError(null);
  }
  useEffect(() => {
    ...
    fetch(examples[example].path, { signal: controller.signal }).then(response => { if (!response.ok) throw new Error('The example could not be loaded.'); return response.json(); }).then(value => {
      if (id === sequence.current) adopt(validateUsageBundle(value), examples[example].label);
  ...
  async function loadFile(selectedFile?: File) {
    ...
    try {
      ...
      const next = format === 'auto' ? loadedBundle(JSON.parse(text)) : validateUsageBundle(importUsage(text, { format, dataset_id: 'local-session', work_item_id: workIdentifier.trim() ? workIdentifier : undefined }));
  ...
  return <main className="usage-explorer">
    <header className="usage-header"><div><span className="usage-wordmark">flAImegraph <span>0.4</span></span><h1>Usage & delivery efficiency</h1></div><nav aria-label="Explorer views"><Link href="/context/">Context</Link><Link href="/operations/">Legacy cost views</Link></nav></header>
    <div className="usage-controls">
      <label>Session<select value={example} onChange={event => { setLoading(true); setError(null); setExample(event.target.value as keyof typeof examples | 'local'); }}><option value="native">{examples.native.label}</option><option value="files">{examples.files.label}</option>{example === 'local' && <option value="local">{filename}</option>}</select></label>
      <label>Import format<select value={format} onChange={event => setFormat(event.target.value)}><option value="auto">Usage / legacy report JSON</option>{['codex','pi','openai','anthropic','gemini','otel'].map(value => <option value={value} key={value}>{value}</option>)}</select></label>
    ...
    {report && bundle && <>
      <section className="usage-meters" aria-label="Recorded usage">{report.meter_totals.filter(item => ['input_tokens','output_tokens','cache_read_input_tokens','reasoning_output_tokens','operation_count','read_bytes','returned_bytes'].includes(item.meter_id)).slice(0, 4).map(item => <button className={metric === item.meter_id ? 'active' : ''} key={item.meter_id} onClick={() => { setMetric(item.meter_id); setPage(0); }}><span>{pretty(item.meter_id)}</span><strong>{item.coverage.known_observations ? number(item.known_total) : 'Unknown'}</strong><small>{item.coverage.unknown_observations ? `${item.coverage.unknown_observations} unavailable observations` : 'Known recorded usage'}{bundle.meters.find(value => value.id === item.meter_id)?.subset_of ? ' · subset' : ''}</small></button>)}</section>
      <Tabs defaultValue="usage"><TabsList className="usage-tabs"><TabsTrigger value="usage">Usage profile</TabsTrigger><TabsTrigger value="efficiency">Efficiency analysis</TabsTrigger><TabsTrigger value="benchmarks">Benchmarks & runway</TabsTrigger></TabsList>
        <TabsContent value="usage">
          {example !== 'local' && <section className="usage-panel"><div className="usage-panel-heading"><div><h2>Flame graph</h2><p>{example === 'native' ? 'Input tokens grouped by model' : 'Recorded operations with real parent ancestry'} · upstream FlameGraph</p></div><a href={`/usage-${example}.svg`} target="_blank" rel="noreferrer">Open interactive graph</a></div><UsageFlamegraph example={example} /></section>}
          <section className="usage-panel"><div className="usage-panel-heading"><div><h2>{pretty(metric)}</h2><p>{total?.coverage.known_observations ? `${number(total.known_total)} ${meter?.unit} · known subtotal` : 'No known additive usage for this measure'}</p></div><div className="usage-inline-controls"><label>Measure<select value={metric} onChange={event => { setMetric(event.target.value); setPage(0); }}>{bundle.meters.map(item => <option value={item.id} key={item.id}>{pretty(item.id)} ({item.unit})</option>)}</select></label><label>Group by<select value={group} onChange={event => { setGroup(event.target.value as UsageGrouping); setSelectedGroup(null); setPage(0); }}>{['model','work_item','task','agent','scope','operation','session'].map(item => <option value={item} key={item}>{pretty(item)}</option>)}</select></label></div></div>
    ...
    <Sheet open={selected !== null} onOpenChange={open => { if (!open) setSelected(null); }}><SheetContent className="usage-detail"><SheetHeader><SheetTitle>{detail?.subject ?? 'Observation'}</SheetTitle><SheetDescription>{detail?.id}</SheetDescription></SheetHeader>{detail && <div className="usage-detail-body"><h3>Work association</h3><p>{detail.work_item_id ?? 'Work identifier unassigned'}</p><small>Task: {detail.task_id ?? 'Unassigned'} · Agent: {detail.agent_id ?? 'Unassigned'} · Dataset: {bundle?.dataset_id}</small><h3>Usage</h3>{Object.entries(detail.measurements).map(([key, value]) => <div className="usage-detail-row" key={key}><strong>{pretty(key)}</strong><span>{number(value.value)} · {value.evidence}</span><small>{value.method} · {value.count_basis} · {value.aggregation}</small></div>)}<h3>Processing dimensions</h3>{Object.entries(detail.dimensions).flatMap(([key, value]) => key === 'extensions' ? Object.entries(value as Record<string, UsageDimensionFact>) : [[key, value] as [string, UsageDimensionFact]]).map(([key, value]) => <div className="usage-detail-row" key={key}><strong>{pretty(key)}</strong><span>{factText(value as UsageDimensionFact)} · {(value as UsageDimensionFact).evidence}</span><small>{(value as UsageDimensionFact).method}</small></div>)}<h3>UTC timestamps</h3>{(['event_at','started_at','ended_at','collected_at'] as const).map(key => <div className="usage-detail-row" key={key}><strong>{pretty(key)}</strong><span>{detail[key]?.value ?? 'Unknown'}</span><small>{detail[key]?.evidence} {detail[key]?.method}</small></div>)}<h3>Source references</h3>{detail.source_refs.map(ref => <p key={`${ref.source_id}:${ref.record}`} className="usage-source-ref">{ref.source_id} · {ref.record}</p>)}</div>}</SheetContent></Sheet>
  </main>;
}
